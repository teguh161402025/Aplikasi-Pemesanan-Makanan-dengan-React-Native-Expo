import { StyleSheet, Text, View, TouchableOpacity, Image, ImageBackground, ScrollView, Button, Modal, Pressable, TouchableNativeFeedback } from 'react-native'
...
const MenuKategori = ({ route, navigation, todos, addTodo, delTodo, UpdateTodo }) => {
  ...
  useEffect(() => {
    ...
    const getMenu = async () => {
      const q = query(collectionRef, where("kategori", "==", route.params.kategori));
      onSnapshot(q, (doc) => {
        let pesanan = []
  ...
  return (
    <View style={{
      ...
      <Text style={{
        fontSize: 17,
        padding: 10
      }} >{route.params.kategoriName}</Text>
      <ScrollView style={{
        maxHeight: "65%"
      }}>
        {
...
export default connect(mapStateToProps, { addTodo, delTodo, UpdateTodo })(MenuKategori);
